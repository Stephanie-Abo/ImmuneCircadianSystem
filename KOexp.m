function dydt = KOexp(t,y,lps0,condition)
...
dydt = zeros(20,1);  % this creates an empty column  
  ...
  % clock genes and proteins
     ...
     dydt(1) =  -dm_per*y(1) + (vmax_per*(1+fold_per*((y(12)/ka_per_cb)^hill_per_cb)))/(1+((y(12)/ka_per_cb)^hill_per_cb)*(1+((y(11)/ki_per_pc)^hill_per_pc)));
%      dydt(2) =  -dm_cry*y(2) + vmax_cry*(1 + fold_cry*(y(12)/ka_cry_cb)^hill_cry_cb)/((1 + (y(12)/ka_cry_cb)^hill_cry_cb*(1 + (y(11)/ki_cry_pc)^hill_cry_pc))*(1 + (y(8)/ki_cry_rev)^hill_cry_rev));
%      dydt(3) =  -dm_rev*y(3) + (vmax_rev*(1+fold_rev*((y(12)/ka_rev_cb)^hill_rev_cb)))/(1+((y(12)/ka_rev_cb)^hill_rev_cb)*(1+((y(11)/ki_rev_pc)^hill_rev_pc)));
%      dydt(4) =  -dm_ror*y(4) + (vmax_ror*(1+fold_ror*((y(12)/ka_ror_cb)^hill_ror_cb)))/(1+((y(12)/ka_ror_cb)^hill_ror_cb)*(1+((y(11)/ki_ror_pc)^hill_ror_pc)));
     dydt(5) =  -dm_bmal*y(5)+ (xP/(xP+lps))*(vmax_bmal*(1+fold_bmal*((y(9)/ka_bmal_ror)^hill_bmal_ror)))/(1+((y(8)/ki_bmal_rev)^hill_bmal_rev)+((y(9)/ka_bmal_ror)^hill_bmal_ror));
     dydt(6) =  -dp_per*y(6) + kp_per*y(1) - (kass_pc*y(6)*y(7) - kdiss_pc*y(11));
%      dydt(7) =  -dp_cry*y(7) + kp_cry*y(2) - (kass_pc*y(6)*y(7) - kdiss_pc*y(11));
%      dydt(8) =  -dp_rev*y(8) + kp_rev*y(3);
%      dydt(9) =  -dp_ror*y(9) + kp_ror*y(4);
     dydt(10) = -dp_bmal*y(10) + kp_bmal*y(5) - kass_cb*y(10) + kdiss_cb*y(12);
     dydt(11) = kass_pc*y(6)*y(7) - kdiss_pc*y(11) - d_pc*y(11);
     dydt(12) = (kass_cb*y(10) - kdiss_cb*y(12)) - d_cb*y(12);
